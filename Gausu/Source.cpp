...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hInstancePrev, LPSTR pCmdLine, int nCmdShow) {
	...
	while (msg.message != WM_QUIT) {
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			XMMATRIX World = XMMatrixTranslation(-0.5f, 0, 0);
...
LRESULT CALLBACK WinProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE:
	{
		...
		D3DCompileFromFile(L"shader.hlsl", NULL, NULL, "PS_SOBEL", "ps_5_0", NULL, 0, &pCompilePS, NULL);
		pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &pPixelShader);
		D3DCompileFromFile(L"shader.hlsl", NULL, NULL, "PS_ORIGIN", "ps_5_0", NULL, 0, &pCompilePS2, NULL);
		...
		pFactory->CreateDecoderFromFilename(L"test.bmp", NULL, GENERIC_READ, WICDecodeMetadataCacheOnDemand, &pDecoder);
		...
		texdec.MiscFlags = 0;
		pDevice->CreateTexture2D(&texdec, NULL, &pTexture);

		// テクスチャを送る
		D3D11_MAPPED_SUBRESOURCE hMappedres;
		pDeviceContext->Map(pTexture, 0, D3D11_MAP_WRITE_DISCARD, 0, &hMappedres);
		...
		pDevice->CreateShaderResourceView(pTexture, &srv, &pTextureSRV);
